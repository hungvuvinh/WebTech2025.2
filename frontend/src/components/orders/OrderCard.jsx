...
export function OrderCard({ order, extra, actions, clickable = false, sellerView = false, productNames = {} }) {
        ...
        <ul className="mt-2 space-y-1 border-t border-[#ebebf0] pt-2">
          {(order.items || []).map((item, i) => (
            <li key={i} className="text-muted-foreground">
              Sản phẩm {item.product_name || productNames[item.product_id] || `…${item.product_id?.slice(-6)}`} × {item.quantity} — {formatPrice(item.unit_price)}
            </li>
          ))}
        </ul>
        {actions}
